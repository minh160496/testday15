...
function bill(kwh) {
    ...
    if (kwh > 200 && kwh <= 300) return (50 * a) + 50 * b + 100 * c + (kwh - 200) * d;
    if (kwh > 300 && kwh <= 400) return (50 * a) + 50 * b + 100 * c + 100 * d + (kwh - 300) * e;
    if (kwh > 400 && kwh <= 400) return (50 * a) + 50 * b + 100 * c + 100 * d + 100 * e + (kwh - 400) * f;
}
...
function countElement(arr) {
    ...
    for (let j = 0; j < arr.length; j++) {
        ...
        for (let k = j + 1; k < arr.length; k++) {
            if (arr[j] === arr[k] && arr[j != undefined]) {
                count[j]++;
                delete arr[j];
                valueOf[j] = arr[j];
            }
...
function sortStudents(students) {
    ...
    //sort theo điểm
    newStudents.sort((a, b) => b.value - a.value);

    //sort theo ngày sinh
    newStudents.sort((a, b) => a.dateOfBirth - b.dateOfBirth);
